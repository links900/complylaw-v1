...
def run_compliance_scan(self, scan_id):
    ...
    for step, progress in steps:
        ...
        if "Connecting" in step:
            result = {"title": "Scanner Ready", "status": "pass", "details": "OK", "module": "System"}
            ...
            result = {"title": "External Data", "status": "pass", "module": "System"}
            ...
            elif "Nmap" in step: result = run_nmap_vuln_scan(domain)
            elif "Nikto" in step: result = run_nikto_scan(domain)
            else: result = {"title": step, "status": "info", "module": "System"}

        # === UPDATE FINDINGS (YOUR EXACT BLOCK) ===
        if not external_results or "Finalizing" not in step:
            ...
            checklist['cookie_banner'] = "cookies" in step.lower() and result["status"] == "pass"
        ...
        scan.scan_log += f"\n[{progress}%] {step}: {result['status'].upper()}"
        scan.save(update_fields=['progress', 'scan_log'])

        async_to_sync(channel_layer.group_send)(
            f"scan_{scan.id}",
            {"type": "scan.update", "progress": progress, "step": step}
        )

        # === FINALIZE LOCAL MODE ===
                # === FINALIZE LOCAL MODE (ONLY RUN ONCE!) ===
        if not external_results:
            ...
            # === FINAL UPDATE ===
            scan.status = 'COMPLETED'  # ← CRITICAL
            scan.completed_at = timezone.now()
            scan.progress = 100
            scan.current_step = "Finalizing report..."

            # === CLEAR OLD [COMPLETE] LINES ===
            scan.scan_log = "\n".join([
                line for line in scan.scan_log.splitlines()
                if not line.strip().startswith("[COMPLETE]")
            ])
